...
def transform():
    ...
    # We store the AVERAGE per-ASKU rate (not the sum) so that
    # suggested batch = avg_rate x target_dos answers the question:
    # "how many units should a single new flavor batch be?"
    tsku_rate_sum:   dict = defaultdict(float)   # tsku -> sum of asku rates
    tsku_rate_count: dict = defaultdict(int)     # tsku -> # of ASKUs with active windows
    for (bsku, type_, flavor), date_map in daily_asku.items():
        ...
        tsku_rate_sum[tsku_key]   += rate
        tsku_rate_count[tsku_key] += 1

    # Average per-ASKU rate per TSKU
    tsku_daily_rate = {
        tsku: round(tsku_rate_sum[tsku] / tsku_rate_count[tsku], 4)
        for tsku in tsku_rate_sum
    }
